...
export default function Home() {
  ...
  {
    _id: 6,
    title: "Building responsive layouts with Tailwind C g responsive layouts with Tailwind C g responsive layouts with Tailwind CSS?",
...
<div className="mt-8 flex w-full flex-col gap-6">
  {questions.length > 0 ? (
    questions.map((que) => (
      <div
        key={que._id}
        className="w-full rounded-xl border border-zinc-200 bg-white p-4 shadow-sm transition-all duration-200 hover:shadow-md dark:border-zinc-700 dark:bg-zinc-900"
      >
        {/* Title */}
        <h2 className="text-base sm:text-lg font-semibold text-zinc-800 dark:text-zinc-100 line-clamp-2 break-words">
          {que.title}
        </h2>
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {que.tags.map((tag) => (
            <span
              key={tag._id}
              className="rounded-md bg-zinc-100 px-2 py-0.5 text-xs font-mono text-zinc-700 dark:bg-zinc-700 dark:text-white"
            >
              {tag.name}
            </span>
          ))}
